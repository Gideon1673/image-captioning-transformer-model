...
class TextDecoderBlock(nn.Module):
    """
    Một Transformer Text Decoder Block.
    ...
    def forward(
        ...
        (
            ...
            text_states=self_attention_residual,
        ...
        (
            ...
            text_states=cross_attention_residual
        )
...
class TextTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        all_self_attention_weights: list[
            torch.Tensor
        ] = []

        all_cross_attention_weights: list[
            torch.Tensor
        ] = []
        ...
        for decoder_block in self.decoder_blocks:
            (
                ...
                text_states=decoder_states,
                visual_features=visual_features,
                padding_mask=padding_mask
            )

            if return_attention_weights:
                all_self_attention_weights.append(
                    self_attention_weights
                )

                all_cross_attention_weights.append(
                    cross_attention_weights
                )

        decoder_features = self.final_norm(
            decoder_states
        )
